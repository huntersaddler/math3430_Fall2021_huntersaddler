...
def matrix_vector_mult(v4_a, m4_a):
  ...
  for index in range(len(m4_a)):
    m4_a[index] = scal_vec_mult(v4_a[index], m4_a[index])
  for index in range(len(m4_a)):
    result = add_vectors(result, m4_a[index])
  return result
...
A3: We will create an empty list named result. Using matrix-vect_mult() to find each linear combination, we will add the 
    product vectors to result using add_vectors()


Alg_5: We will call this algorithm matrix_matrix_mult(matrix, matrix)

Step 1: Create empty list named result
...
This is not running correctly but i can not figure out what i have done wrong. The first pass on the for loop works 
correctly and rewrites the first column vector in result, but the next passes of the for loop do not multiply to the 
right answer. However, If asked to return such as
 
  result = [0 for element in m5_b]
  result[index] = matrix_vector_mult(m5_a[1], m5_a)
  
  the program seems to be multiplying correctly. So i am either iterating or adding the vectors to the result matrix
  wrong. Hopefully something simple since the previous functions seem to be working
...
# add_vectors(testv_4a,testv_4b) should output [6,2,4]
...
testv_1a = [4, 3, 1, 7, 2]
tests_1a = 2
#scal_vec_mult(tests_1a, testv_1a) should output [8, 6, 2, 14, 4]
print('Test Output for scal_vec_mult: ' + str(scal_vec_mult(tests_1a, testv_1a)))
print('Should have been [8, 6, 2, 14, 4]')

testv_1b = [3, 0, 10, 111]
tests_1b = 3
#scal_vec_mult(tests_1b, testv_1b) should output [9, 0, 30, 333]
...
tests_2a = 2
#scal_matrix_mult(tests_2a, testv_2a) should output [[4, 6], [12, 2], [8, 6]]
...
tests_2b = 3
#scal_matrix_mult(tests_2b, testv_2b) should output
...
#matrix_matrix_add(testm_3a, testm_3b) should output [[2, 3], [3, 4], [4, 5]]
...
#matrix_matrix_add(testm_3c, testm_3d) should output [[2, 2], [8, 3], [30, 5], [200, 200]]
